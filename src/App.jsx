...
function App() {
  ...
  const exportPendingToExcelAndClear = async () => {
    ...
    // Columns: User Email, User Name, Amount (INR), Method, Bank Account / UPI ID, IFSC Code, Bank Name, Account Holder Name, Available Balance After Withdrawal, Date, Status
    const headers = [
      "Request ID",
      "Client Email",
      "Client Name",
      ...
      "Bank Name",
      "Account Holder Name",
      "Available Balance After Withdrawal (INR)",
      "Date Requested",
      "Status"
    ];

    const rows = pending.map(w => {
      const methodLabel = w.paymentMethod === 'upi_withdrawal' ? 'UPI (Live)' :
                         w.paymentMethod === 'upi_withdrawal_sim' ? 'UPI (Simulated)' :
                         w.paymentMethod === 'bank_withdrawal' ? 'Bank (Live)' :
                         w.paymentMethod === 'bank_withdrawal_sim' ? 'Bank (Simulated)' : w.paymentMethod;
      
      const destinationVal = w.upiId || w.bankAccount || 'N/A';
      
      return [
        w.id,
        w.user?.email || 'N/A',
        w.user?.name || w.user?.email?.split('@')[0] || 'N/A',
        w.amount,
        methodLabel,
        `="${destinationVal}"`, // Force Excel to treat account numbers as text (prevent scientific notation/leading zero loss)
        w.ifsc || 'N/A',
        w.bankName || 'N/A',
        w.accountHolder || 'N/A',
        w.availableBalanceAfter !== null && w.availableBalanceAfter !== undefined ? w.availableBalanceAfter : 'N/A',
        new Date(w.createdAt).toISOString().slice(0, 16).replace('T', ' '),
        w.status
      ];
    });
              ...
              <div className="modal-body">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginBottom: '24px' }}>
                    <div style={{ display: 'flex', gap: '16px' }}>
                      <div className="funding-input-group" style={{ flex: 1 }}>
                        ...
                        <input type="text" placeholder="Beneficiary Holder Name" value={accountHolderName} onChange={(e) => setAccountHolderName(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.2)', color: '#fff', outline: 'none' }} />
                      </div>
                      <div className="funding-input-group" style={{ flex: 1 }}>
                        <label>Bank Name</label>
                        <input type="text" placeholder="e.g. State Bank of India" value={bankName} onChange={(e) => setBankName(e.target.value)} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.2)', color: '#fff', outline: 'none' }} />
                      </div>
                    </div>
                    <div className="modal-input-row" style={{ display: 'flex', gap: '16px' }}>
                      <div className="funding-input-group" style={{ flex: 1 }}>
                ...
                <RazorpayButton 
                  ...
                  payoutDetails={
                    ...
                    (bankName && accountHolderName && accountNumber && ifscCode ? { accountName: accountHolderName, bankName, accountNumber, ifsc: ifscCode.toUpperCase() } : null)
                  }
                  onSuccess={(data) => {
